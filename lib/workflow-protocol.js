...
function isQuestionBacklog(value) {
  return Array.isArray(value) && idsAreUnique(value)
    && value.every((question) => isNonEmptyString(question.question));
}
...
export function isV2State(state, manifest, workflowId) {
  if (!isRecord(state)
    ...
    || (state.evidence.questionBacklog !== undefined && !isQuestionBacklog(state.evidence.questionBacklog))
    || (state.evidence.questionAnswers !== undefined && !Array.isArray(state.evidence.questionAnswers))
...
export function questionBacklog(state) {
  return Array.isArray(state.evidence.questionBacklog) ? state.evidence.questionBacklog : undefined;
}

export function hasUnresolvedQuestions(state) {
  const backlog = questionBacklog(state);
  // Existing v2 workflows already in planning predate the backlog. Preserve
  // their state without a migration; intake and refinement must initialize it.
  return !backlog ? state.phase !== "planning" : backlog.length > 0;
}

export function canEnterPlanning(state) {
  return !hasUnresolvedQuestions(state) && state.evidence.sharedUnderstanding === true;
}

...
Do not modify source files, redefine criteria, or delegate further work. Never report a check as passing unless you ran it and read its output. If a check cannot run, state why and what remains unverified. For a selected workflow, verify the approved manifest version criterion by criterion and provide the exact evidence needed for an append-only verification receipt; do not declare completion without it.
